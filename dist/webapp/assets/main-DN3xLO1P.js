...
`;const s={question:Math.max(10,...t.map(i=>i.question.length)),answer1:Math.max(G.length,...t.map(i=>(i.answer1||"-").length)),answer2:Math.max(j.length,...t.map(i=>(i.answer2||"-").length))};s.question=Math.min(s.question,30),s.answer1=Math.min(s.answer1,25),s.answer2=Math.min(s.answer2,25),n+=se("Frage",s.question)+" | ",n+=se(G,s.answer1)+" | ",n+=se(j,s.answer2)+" | ",n+=`Gemeinsamkeit
`,n+="-".repeat(s.question)+"-+-",n+="-".repeat(s.answer1)+"-+-",n+="-".repeat(s.answer2)+"-+-",n+="-".repeat(20)+`
`;for(const i of t){const a=fe(i.comment);n+=se(ue(i.question,s.question),s.question)+" | ",n+=se(ue(i.answer1||"-",s.answer1),s.answer1)+" | ",n+=se(ue(i.answer2||"-",s.answer2),s.answer2)+" | ",n+=(a||"")+`
`}return n+=`
Ich freue mich über eure Rückmeldung!
`,n}function se(e,t){return e.length>=t?e.substring(0,t):e+" ".repeat(t-e.length)}function ue(e,t){return e?e.length<=t?e:e.substring(0,t-2)+"..":""}function ts(){const t=y.filter(s=>s.included).filter(s=>s.answer1||s.answer2);let n=`<!--StartFragment-->
...
`;if(n.commonalities&&n.commonalities.length>0){const r={question:Math.max(10,...n.commonalities.map(o=>o.question.length)),answer1:Math.max(s.length,...n.commonalities.map(o=>(o.answer1||"-").length)),answer2:Math.max(i.length,...n.commonalities.map(o=>(o.answer2||"-").length))};r.question=Math.min(r.question,30),r.answer1=Math.min(r.answer1,25),r.answer2=Math.min(r.answer2,25),a+=ie("Frage",r.question)+" | ",a+=ie(s,r.answer1)+" | ",a+=ie(i,r.answer2)+" | ",a+=`Gemeinsamkeit
`,a+="-".repeat(r.question)+"-+-",a+="-".repeat(r.answer1)+"-+-",a+="-".repeat(r.answer2)+"-+-",a+="-".repeat(20)+`
`;for(const o of n.commonalities)a+=ie(Ne(o.question,r.question),r.question)+" | ",a+=ie(Ne(o.answer1||"-",r.answer1),r.answer1)+" | ",a+=ie(Ne(o.answer2||"-",r.answer2),r.answer2)+" | ",a+=(o.commonality||"")+`
`}a+=`
Ich freue mich über eure Rückmeldung!
`,navigator.clipboard.writeText(a).then(()=>{gt("Text kopiert!")}).catch(()=>{alert("Fehler beim Kopieren")})}function ie(e,t){return e.length>=t?e.substring(0,t):e+" ".repeat(t-e.length)}function Ne(e,t){return e?e.length<=t?e:e.substring(0,t-2)+"..":""}function ft(e){if(!e||typeof e!="string")return"Partner*in";const t=e.match(/\(([^)]+)\)/);if(t){const s=t[1].trim().split(/[\s,]+/)[0];if(s&&s.length>1&&!/^(locals?|einwander|interview|gespräch)/i.test(s))return s}if(!/^(aufnahmegespräch|interview|gespräch)/i.test(e)){const n=e.split(/[\s,]+/)[0];if(n&&n.length>1)return n}return"Partner*in"}function gt(e){let t=document.getElementById("successToast");t||(t=document.createElement("div"),t.id="successToast",t.className="success-toast",document.body.appendChild(t)),t.textContent=e,t.classList.add("visible"),setTimeout(()=>t==null?void 0:t.classList.remove("visible"),2e3)}function os(e){const t=new Date(e.created).toLocaleDateString("de-DE"),n=Ye(e.matchScore);return`
